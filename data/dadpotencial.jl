...
function fa1(NOS, ns=500)
    ...
    for i = 1:nc
        ...
        for n = 1:ns
            ...
            inc = ((((-1)^(n + 1)) + 1) / n) * sin(n * π * x) * ((1 - exp(-2n * π * y)) / (n * π * (1 + exp(-2n * π)))) * exp(n * π * (y - 1))
            if isnan(inc)
                break
            end
            T_ana += inc
...
function fa2(NOS, ns=500)
    ...
    for i = 1:nc
        ...
        T_ana = 0.0
        for n = 1:ns
            inc = ((sin(n * π * x / 2)) * (((2 / (n * π)) * (((-1)^(n + 1) + 1) / (((n * π) / 2) * cosh(n * π / 2))) + (8 / ((n^2) * (π^2))) * sin(n * π / 2) * tanh(n * π / 2)) * sinh(n * π * y / 2) - ((8 / ((n^2) * (π^2))) * sin(n * π / 2) * cosh(n * π * y / 2))))
            if isnan(inc)
                # @show n, inc
                break
            end
...
function fa3(NOS, ns=1000)
    ...
    for i = 1:nc
        ...
        for n = 1:ns
            ...
            # end
            inc = ((((-1)^(n + 1)) + 1) / n) * sin(n * π * x) * ((1 - exp(-2n * π * y)) / (1 - exp(-2n * π))) * exp(n * π * (y - 1))
            if isnan(inc)
                #  @show i, n, inc
                break
            end
