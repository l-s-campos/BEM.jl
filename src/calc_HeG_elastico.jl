function calc_HeG(dad::elastico, npg=8; interno=false, subcheia=false)
  ...
  for i = 1:n
    ...
    for elem_j in dad.ELEM  #Laço dos elementos
      ...
      #   xi0 = elem_j.ξs[no_pf]
      #   # h, g = integraelemsing_num(pf, x, elem_j, dad, xi0)
      # end
      # @infiltrate
...
function calc_tens_cont(dad::elastico_aniso_iga, u, t)
  ...
  for elem_j in dad.ELEM  #Laço dos elementos
    ...
    for i in elem_j.sing
      ...
      vector = [s11 - rigidez_loc[1, 2] * e22
        -rigidez_loc[2, 2] * e22
...
function calc_tens_cont(dad::elastico, uc, t)
  ...
  for elem_j in dad.ELEM  #Laço dos elementos
    ...
    for i in 1:size(elem_j.indices, 1)
      ...
      tens_nt[elem_j.indices[i], :] = [s11, s12]  #normal,  tangente, normal tangente    
    end
  end
  tens_cont, tens_nt
end
  ...
  for k = 1:2
    for i = 1:2
      for j = 1:2
        ...
        t1 = 2 * drdn * (fat2 * kro[i, j] * dr[k] + v * (kro[i, k] * dr[j] + kro[j, k] * dr[i]) - 4 * dr[i] * dr[j] * dr[k])
        t2 = 2 * v * (n[i] * dr[j] * dr[k] + n[j] * dr[i] * dr[k])
        t3 = fat2 * (2 * n[k] * dr[i] * dr[j] + n[j] * kro[i, k] + n[i] * kro[j, k]) - (1 - 4 * v) * n[k] * kro[i, j]
...
vonmises(tens) = sqrt.(tens[:, 1] .^ 2 - tens[:, 1] * tens[:, 2] + tens[:, 2] .^ 2 + 2 * tens[:, 3] .^ 2)
...
function Monta_M_RIMd(dad::Union{elastico,elastico_aniso}, npg)
  ...
  for i = 1:n_pontos#Laço dos pontos radiais
    A[2i-1:2i, 2i-1:2i] .= 0
    A[2i-1:2i, 2i-1:2i] = -[sum(A[2i-1:2i, 1:2:end], dims=2) sum(A[2i-1:2i, 2:2:end], dims=2)] + M2[2i-1:2i, :]
...
function Monta_dM_RIMd(dad::Union{elastico,elastico_aniso}, npg=10)
  ...
  for i = 1:n_pontos #Laço dos pontos radiais
    A[3i-2:3i, 2i-1:2i] = -[sum(A[3i-2:3i, 1:2:end], dims=2) sum(A[3i-2:3i, 2:2:end], dims=2)] + M2[3i-2:3i, :]
  end
...
function calc_gap(dad::DadosBEM)
  ...
  # h(número do nó, flag do contato) 
  # flag do contato = 1 => O nó pode entrar em contato
  # flag do contato = 0 => O nó nunca vai entrar em contato
...
function muda_nt!(dad, H, G)
  ...
  for i in 1:nc(dad)
    R[2i-1:2i, 2i-1:2i] = [dad.normal[i, 1] dad.normal[i, 2]; -dad.normal[i, 2] dad.normal[i, 1]]
  end
  H .= H * R
  G .= G * R
  return nothing
...
function verifica_contato_sem_atrito(x, h, dad)
...
function aplica_contato_sem_atrito!(h, contato, A, b, dad)
  ...
  for k in 1:nnoscontato  # Percorre todos os elementos que podem entrar
    ...
    # A[[postx, posty], [posux, posuy]]
    # A[[postx, posty], [postx, posty]]
    if tipocontato == 1  # Zona livre de contato
      A[posty, posty] = 1#dad.normal[no_contato, 1]  # tt = 0
      A[posty, postx] = 0#-dad.normal[no_contato, 2]
      A[posty, posux] = 0
      A[posty, posuy] = 0
      A[postx, posty] = 0#dad.normal[no_contato, 2]
      A[postx, postx] = 1#dad.normal[no_contato, 1]
      A[postx, posux] = 0  # tn = 0
      A[postx, posuy] = 0
      b[postx] = 0  # ux = gap        
    elseif tipocontato == 3  # Zona em contato e adesão
      A[posty, posty] = 0#dad.normal[no_contato, 1]  # tt = 0
      A[posty, postx] = 0#-dad.normal[no_contato, 2]
      A[posty, posux] = 1
      A[posty, posuy] = 0
      A[postx, posty] = 0
      A[postx, postx] = 0
      A[postx, posux] = 1#dad.normal[no_contato, 1]  # un = gap
      A[postx, posuy] = -1#dad.normal[no_contato, 2]
      b[postx] = h[2][k]  # un = gap
    else #escorregamento
      A[posty, posty] = dad.k.mi * sign(tipocontato)#dad.normal[no_contato, 1]  # tt = 0
      A[posty, postx] = 1#-dad.normal[no_contato, 2]
      A[posty, posux] = 0
      A[posty, posuy] = 0
      A[postx, posty] = 0
      A[postx, postx] = 0
      A[postx, posux] = 0#dad.normal[no_contato, 1]  # un = gap
      A[postx, posuy] = -1#dad.normal[no_contato, 2]
      b[postx] = h[2][k]  # un = gap
    end
  end
...
function Contato_sem_atrito_NL(x0, p, dad)
  A2, b2, h = p
  # Verifica a condição de contato de cada nó
  contato = verifica_contato_sem_atrito(x0, h, dad)
  # Aplica a condição de contato em cada nó
  aplica_contato_sem_atrito!(h, contato, A2, b2, dad)
  A2 * x0 - b2
end

function Contato_sem_atrito_NL_newton(dad, x0, A2, b2, h; maxiter=10, tol=1e-8)
