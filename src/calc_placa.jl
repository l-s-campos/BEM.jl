function Compute_Material_Placa(Material)
  ...
  e[2] = imag(mi[2])
  # @infiltrate
  (d=d, e=e, D11=D11, D22=D22, D66=D66, D12=D12, D16=D16, D26=D26, Material=Material)
end
...
function calsolfund(pg, pf, n, nf, dad::Union{placa_fina}, pre)
  ...
  R, S, d2Rdx2, d2Rdxdy, d2Rdy2, d2Sdx2, d2Sdxdy, d2Sdy2, d3Rdx2dy, d3Rdx3, d3Rdxdy2, d3Rdy3, d3Sdx2dy, d3Sdx3, d3Sdxdy2, d3Sdy3, d4Rdx2dy2, d4Rdx3dy, d4Rdx4, d4Rdxdy3, d4Rdy4, d4Sdx2dy2, d4Sdx3dy, d4Sdx4, d4Sdxdy3, d4Sdy4, dRdx, dRdy, dSdx, dSdy = pre
  rs = pg - pf
  nx = n[1]
  ny = n[2]
  m1 = nf[1]
...
function calsolfund2(pg, pf, n, nf, dad::Union{placa_fina}, pre)
  ...
  d2wdmds = (d2wdx2 * m1 * s1 + d2wdxdy * (m1 * s2 + m2 * s1) + d2wdy2 * m2 * s2) / dad.k.D22
  #  @infiltrate
  u = [w -dwdn
    dwdm -d2wdndm]
  p = [vn -mn
...
function integraelem(pf, nf, x, eta, w, elem, dad::Union{placa_fina,placa_fina_isotropica}, pre)
...
function integraelem2(pf, nf, x, eta, w, elem, dad::Union{placa_fina,placa_fina_isotropica}, pre)
  h = zeros(Float64, 2, 2 * size(elem))
  g = zeros(Float64, 2, 2 * size(elem))
...
function compute_Rw(pf, nf, dad::placa_fina, pre)
  m1 = nf[1]
  m2 = nf[2]
  R, S, d2Rdx2, d2Rdxdy, d2Rdy2, d2Sdx2, d2Sdxdy, d2Sdy2, d3Rdx2dy, d3Rdx3, d3Rdxdy2, d3Rdy3, d3Sdx2dy, d3Sdx3, d3Sdxdy2, d3Sdy3, dRdx, dRdy, dSdx, dSdy, extra = pre
...
function compute_q(pf, nf, x, eta, Gauss_w, elem, dad::placa_fina, pre)
  ...
  for k = 1:size(Gauss_w, 1)# Integração da carga distribuída sobre o domínio
    ...
    # Clocal = 1
    Clocal = A * xf + B * yf + C
    intRkrdr, intSkrdr, intRABrdr, intSABrdr, intdRdxABrdr, intdRdyABrdr, intdSdxABrdr, intdSdyABrdr, intdRdxrdr, intdRdyrdr, intdSdxrdr, intdSdyrdr, extra = pre
    ...
    int1 = 1 / (8 * pi * dad.k.D22) * (C1 * (intRkrdr[1] + intRABrdr[1]) + C2 * (intRkrdr[2] + intRABrdr[2]) + C3 * ((intSkrdr[1] + intSABrdr[1]) - (intSkrdr[2] + intSABrdr[2]))) * nr
...
function passonotempo(dad, A, B, q, bc_val, M, dM, npg)
  ...
  @showprogress "Resolvendo no tempo" for idt = 1:dad.k.n_dt
    ...
    d2wdt2 = 1 / dad.k.dt^2 * (2 * listdisp - 5 * U_ant[:, 3] + 4 * U_ant[:, 2] - U_ant[:, 1])

    der2M = [dM2dx2 * d2wdt2 dM2dy2 * d2wdt2 dM2dxdy * d2wdt2] * dad.k.rho * dad.k.thickness
...
function simula_placa_tempo(d::Dict)
  ...
  NPY = NPX
  tdad = @timed dad = format_dad(getfield(Main, Symbol(problema))(nelem, 3, nt), NPX, NPY, canto=true) # dados
...
function aplicaT(G, iT, T, dNs, M3, N)
  ...
  Nit = 0 * G
  # @infiltrate
  Nit[:, 1:2:2n] = G[:, 1:2:2n] .* T[:, 2]' * dNs - G[:, 2:2:2n] .* T[:, 1]' - iT[:, 1:n] .* T[:, 2]' + M3[:, 1:3:3n] .* N[1:n, 1]' + M3[:, 2:3:3n] .* N[1:n, 2]' + M3[:, 3:3:3n] .* N[1:n, 3]'
  Nit[:, 2n+1:n+n2] = M3[:, 3n+1:3:3n2] .* N[n+1:n2, 1]' + M3[:, 3n+2:3:3n2] .* N[n+1:n2, 2]' + M3[:, 3n+3:3:3n2] .* N[n+1:n2, 3]'
...
function aplicaT(dad::Union{placa_fina,placa_fina_isotropica}, G, T, dNs, M2, N)
  ...
  F, Fx1, Fy1 = BEM.montaFs([dad.NOS; dad.pontos_internos; dad.k.cantos[:, 2:3]], [dad.NOS; dad.pontos_internos])
  F1, Fx, Fy = BEM.montaFs([dad.NOS; dad.pontos_internos; dad.k.cantos[:, 2:3]], [dad.NOS; dad.pontos_internos; dad.k.cantos[:, 2:3]])
  # @infiltrate
  Ni = F * N
...
function aplicaT(dad::Union{placa_fina,placa_fina_isotropica}, M, N)
  # @infiltrate
  n2 = size(N, 1)
  F1, Fx1, Fy1 = BEM.montaFs([dad.NOS; dad.pontos_internos; dad.k.cantos[:, 2:3]], [dad.NOS; dad.pontos_internos])
  F, Fx, Fy = BEM.montaFs([dad.NOS; dad.pontos_internos; dad.k.cantos[:, 2:3]], [dad.NOS; dad.pontos_internos; dad.k.cantos[:, 2:3]])
  Fyx = Fy * Fx
  Fxy = Fx * Fy
  Fyy = Fy * Fy
  Fxx = Fx * Fx
  ...
  K = (Nx[:, 1] + Ny[:, 3])' .* Fx + (Nx[:, 3] + Ny[:, 2])' .* Fy + ((F1 * N[:, 1])' .* Fxx + (F1 * N[:, 3])' .* Fxy + (F1 * N[:, 2])' .* Fyy + (F1 * N[:, 3])' .* Fyx)

  k2 = zeros(2nc(dad) + ni(dad) + size(dad.k.cantos, 1), 2nc(dad) + ni(dad) + size(dad.k.cantos, 1))
...
function criatensoes(dad, Nexato)
  ...
  for elem_j in dad.ELEM  #Laço dos elementos
    ...
    for i in 1:length(elem_j.indices)
      ...
      #      txy=valuex*ny;
      # @infiltrate
      tens_nt[elem_j.indices[i], :] = [Nexato[1] * normal[1] + Nexato[3] * normal[2], Nexato[2] * normal[2] + Nexato[3] * normal[1], Nexato[3] * normal[2]]  #normal,  tangente
    end
  end
  tens_nt, tens
end
function criatensoesnt(dad, tens)
  ...
  for elem_j in dad.ELEM  #Laço dos elementos
    ...
    for i in 1:length(elem_j.indices)
      ...
      # @infiltrate
      tens_nt[elem_j.indices[i], :] = [tens[i, 1] * normal[1] + tens[i, 3] * normal[2], tens[i, 2] * normal[2] + tens[i, 3] * normal[1]]  #normal,  tangente
    end
  end
  tens_nt
end
function calsolfund2(pg, pf, n, nf, dad::Union{placa_fina_isotropica}, pre)
  ...
  dvndm = -1 / (4 * pi * r^2) * (2 * (1 - ni) * sr * (sr * mn + 2 * nr * ms - 4 * sr * mr * nr) + (3 - ni) * (-mn + 2 * mr * nr))
...
function integraelemsing(x, dad::placa_fina_isotropica, xi0)
  ...
  for j = 1:3
    # Integrais das derivadas segundas da solução fundamental
    d2wdx2 = 1 / (8 * pi * D) * ((1 + cos(2 * theta)) * intN[j] * L / 2 + 2 * NlogLqqo[j])
    d2wdxdy = 1 / (8 * pi * D) * sin(2 * theta) * intN[j] * L / 2
    d2wdy2 = -1 / (8 * pi * D) * ((-1 + cos(2 * theta)) * intN[j] * L - 2 * NlogLqqo[j])

    # Integrais das derivadas terceira da solução fundamental
    d3wdx3 = 1 / (4 * pi * D) * cos(theta) * (3 - 2 * cos(theta)^2) * 2 * intNsr[j] / L * L / 2
    d3wdx2dy = 1 / (4 * pi * D) * sin(theta) * (1 - 2 * cos(theta)^2) * 2 * intNsr[j] / L * L / 2
    d3wdxdy2 = 1 / (4 * pi * D) * cos(theta) * (1 - 2 * sin(theta)^2) * 2 * intNsr[j] / L * L / 2
    d3wdy3 = 1 / (4 * pi * D) * sin(theta) * (3 - 2 * sin(theta)^2) * 2 * intNsr[j] / L * L / 2

    # Integrais das derivadas quarta da solução fundamental
    d4wdx4 = 1 / (4 * pi * D) * (8 * cos(theta)^4 - 12 * cos(theta)^2 + 3) * (4 * intNsr2[j] / L^2) * L / 2
    d4wdx3dy = 1 / (4 * pi * D) * (-6 * cos(theta) * sin(theta) + 4 * sin(theta) * cos(theta)^3 + 4 * cos(theta)^3 * sin(theta)) * (4 * intNsr2[j] / L^2) * L / 2
    d4wdx2dy2 = 1 / (4 * pi * D) * (8 * (cos(theta) * sin(theta))^2 - 1) * (4 * intNsr2[j] / L^2) * L / 2
    d4wdxdy3 = 1 / (4 * pi * D) * (-6 * cos(theta) * sin(theta) + 4 * cos(theta) * sin(theta)^3 + 4 * cos(theta) * sin(theta)^3) * (4 * intNsr2[j] / L^2) * L / 2
    d4wdy4 = 1 / (4 * pi * D) * (8 * sin(theta)^4 - 12 * sin(theta)^2 + 3) * (4 * intNsr2[j] / L^2) * L / 2
...
function placa_grande1(dad::placa_fina_isotropica, dadpe, ntime, npg, e=0.5)
  ...
  ~, Fx, Fy = BEM.montaFs([dad.NOS; dad.pontos_internos; dad.k.cantos[:, 2:3]], smooth=1e-8)
  ...
  ws = zeros(2 * size(dad.NOS, 1) + size(dad.pontos_internos, 1) + size(dad.k.cantos, 1), ntime)
  wind = [1:2:2*nc(dad); 2*nc(dad)+1:2*size(dad.NOS, 1)+size(dad.pontos_internos, 1)+size(dad.k.cantos, 1)]
  @showprogress "Passo não-linear" for i = 1:ntime
    ...
    N_nonlin = (1 - dadpe.k.nu) / 2 * CB * [dwdx .^ 2 + somadw dwdy .^ 2 + somadw dwdx .* dwdy]
    dNndx = Fx * N_nonlin
    dNndy = Fy * N_nonlin
    DN = [dNndx[:, 1] + dNndy[:, 3] dNndx[:, 3] + dNndy[:, 2]]'[1:2*(size(dad.NOS, 1)+size(dad.pontos_internos, 1))]
    qnl = Mpe * DN
...
function corrige_derivada(w, dad, dwdx, dwdy)
  for elem_j in dad.ELEM  #Laço dos elementos
    ...
    for i in 1:size(elem_j.indices, 1)
      N, dN = calc_fforma(elem_j.ξs[i], elem_j)
      dxdqsi = dN' * x
      dgamadqsi = norm(dxdqsi)  # dΓ/dξ = J(ξ) Jacobiano
      sx = dxdqsi[1] / dgamadqsi # vetor tangente dx/dΓ
