...
function aplicaCDC_u(Ht, Gt, dad)
  ...
  b = zeros(n)
  Gu = -dad.k * Gt[:, troca] * (Fx .* normal_fonte[troca, 1] + Fy .* normal_fonte[troca, 2])
  # @infiltrate
  A = Ht - Gu
...
function aplicaCDC_alpha(Ht, Gt, dad)
  normal_fonte = calc_normais(dad)
  F, dFdx, dFdy, dFdxx, dFdyy, dFdxy = BEM.montaF([dad.NOS; dad.pontos_internos], [dad.NOS; dad.pontos_internos])
  dFdn = (dFdx[1:nc(dad), :] .* normal_fonte[:, 1] + dFdy[1:nc(dad), :] .* normal_fonte[:, 2])
  Ga = -dad.k * Gt * dFdn
  Ha = Ht * F
