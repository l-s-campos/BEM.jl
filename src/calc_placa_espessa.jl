...
function calsolfund(pg, pf, n, dad::placa_espessa_isotropica)
  ...
  U11 = 1 / (k2 * (1 - v)) * (8 * Bz - (1 - v) * (2 * log(z) - 1) - (8 * Az + 2 * (1 - v)) * DRx^2)
  U12 = -1 / (k2 * (1 - v)) * (8 * Az + 2 * (1 - v)) * DRx * DRy
  U13 = 1 / k2 * (2 * log(z) - 1) * R * DRx
  U21 = U12
  U22 = 1 / (k2 * (1 - v)) * (8 * Bz - (1 - v) * (2 * log(z) - 1) - (8 * Az + 2 * (1 - v)) * DRy^2)
...
function integraelem(pf, x, eta, w, elem, dad::Union{placa_espessa,placa_espessa_isotropica})
...
function integraelemsing_num(pf, x, elem, dad::Union{placa_espessa_isotropica}, xi0, npg=30)
  h = zeros(Float64, 3, 3 * size(elem))
  g = zeros(Float64, 3, 3 * size(elem))
  # Cij = zeros(Float64, 3, 3)
...
function compute_q(pf, x, eta, Gauss_w, elem, dad::Union{placa_espessa,placa_espessa_isotropica})
...
function calc_m(x, pf, pr, qsi1, w1, elem, dad::Union{placa_espessa,placa_espessa_isotropica})
...
function calcula_F(pr, pf, pg, n, qsi, w, dad::Union{placa_espessa,placa_espessa_isotropica}) #
  npg = length(w)
  R = (pg' - pf)
  r = norm(R)
...
function Monta_M_RIMd(dad::Union{placa_espessa,placa_espessa_isotropica}, npg1=10)
  ...
  for i = 1:n_pontos #Laço dos pontos radiais
    A[3i-2:3i, 3i-2:3i] .= 0
    A[3i-2:3i, 3i-2:3i] = -[sum(A[3i-2:3i, 1:3:end], dims=2) sum(A[3i-2:3i, 2:3:end], dims=2) sum(A[3i-2:3i, 3:3:end], dims=2)] + M[3i-2:3i, :]
  end
  A
end
