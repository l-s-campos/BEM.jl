...
function calsolfund_hiper(r, n, nf, dad::helmholtz)
    ...
    Qast =
        dad.k.FR / dad.k.CW * im / 4 / R * hankelh1(1, ZR) * dot(nf, n) -
        (dad.k.FR / dad.k.CW)^2 * im / 4 * hankelh1(2, ZR) * dot(r, n) / R * dot(r, nf) / R
...
function corrigediags!(H, G, dad, interno = true)
    ...
    ps = [((xmax + xmin) / 2 .+ d * l * sin.(range(0, 2pi, length = nf + 1)[1:end-1])) (
        (ymax + ymin) / 2 .+ d * l * cos.(range(0, 2pi, length = nf + 1)[1:end-1])
    )]
    ...
    for e in dad.ELEM, ii = 1:np, ifonte = 1:nf
        qsf[e.indices[ii], ifonte], tsf[e.indices[ii], ifonte] = BEM.calsolfund(
            dad.NOS[e.indices[ii], :] - ps[ifonte, :],
            dad.normal[e.indices[ii], :],
            dad,
        )
    ...
    for e in dad.ELEM, ii = 1:np
        ...
        if interno
            ...
            hgd =
                conj.([tsf[e.indices, :]; -qsf[e.indices, :]]') \
                (-v[e.indices[ii], :] - tsf[e.indices[ii], :])
        end
