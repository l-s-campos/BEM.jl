function mostra_geometria(dad)
  ...
  for el in dad.ELEM
    ...
    xi = (ps[2, 1] - ps[1, 1]) / 2
    yi = (ps[2, 2] - ps[1, 2]) / 2
    lines!(pos, [ps[1, 1] - yi, ps[1, 1] + yi], [ps[1, 2] - xi, ps[1, 2] + xi], color=:black, linestyle=:dashdot)

    xe = (ps[end, 1] - ps[end-1, 1]) / 2
    ye = (ps[end, 2] - ps[end-1, 2]) / 2
    lines!(pos, [ps[end, 1] - ye, ps[end, 1] + ye], [ps[end, 2] - xe, ps[end, 2] + xe], color=:black, linestyle=:dashdot)
    if typeof(dad) == elastico
      if el.tipoCDC[1] == 0
        scatter!(pos, x[:, 1], x[:, 2], color=:blue, marker=:rtriangle, markersize=30)
